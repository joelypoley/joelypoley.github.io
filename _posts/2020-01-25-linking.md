...
To tell the compiler to link with a `.a` in `/usr/local/lib`, pass the flag `-l<name of library>` to the compiler (type `gcc -print-search-dirs` to see the full link path).
...
Note the `lib` prefix is omitted, there is no space between `-l` and `fftw` and the `-lfftw3` flag is after the source file which uses it.
...
Many statically typed languages such as C, Rust, Swift follow the same model as C++, separate compilation of source files followed by linking.
...
--------------------------------------------------------------
<b id="f1">1</b> C does not require an explicit declarations, C++ does.
